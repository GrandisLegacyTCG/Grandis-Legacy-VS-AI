...
assert.equal(require('../package.json').version,'6.39.0');
...
must(guide.includes("interactionStateCheck:function(){var current=bridge.getState();return !!current&&(current.turn!=='PLAYER'||current.phase!==phase);}"),'Next Phase instruction must wait for a real phase transition');
must(tutorialApp.includes('isRuntimePresentationBusy:function()'),'Tutorial bridge runtime-presentation gate missing');
console.log('PASS VS AI v6.39 / Tutorial v0.65 Shard teaching and terminology');
