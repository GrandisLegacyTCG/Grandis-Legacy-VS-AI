...
const tests=[
  ...
  'run-v642-candidate14-browser.cjs'
];
...
(async()=>{for(const file of tests){console.log('RUN browser gate '+file);await run(file);console.log('DONE browser gate '+file);await new Promise(r=>setTimeout(r,1200));}console.log('PASS browser suite: current Desktop baseline + Candidate (14) real-touch Tablet gates completed.');})().catch(err=>{console.error(err&&err.stack||err);process.exit(1);});
