...
must(pkg.version==='6.35.0','VS AI package must be v6.35.0');
must(tutPkg.version==='0.61.0','Tutorial package must be v0.61.0');
...
must(app.includes('Grandis Legacy VS AI v6.35'),'VS AI v6.35 marker missing');
must(app.includes('One Source v1.8.2')&&app.includes('Runtime Data v0.15.0'),'current authority marker missing');
must(index.includes('gl-vs-ai-6.35'),'VS AI v6.35 cache marker missing');
...
must(stack.local_ai==='v6.35'&&stack.tutorial==='v0.61'&&stack.pvp_railway==='v3.41','cross-release metadata stale');
...
console.log('PASS VS AI v6.35 current release: Source Stack v1.8.2, corrected Shard payment batches, Triple Shot pre-play validation, 200 cards, current assets and gameplay locks.');
