...
must(pkg.version==='6.33.0','VS AI package must be v6.33.0');
must(tutPkg.version==='0.59.0','Tutorial package must be v0.59.0');
...
must(app.includes('Grandis Legacy VS AI v6.33'),'VS AI v6.33 marker missing');
must(app.includes('One Source v1.8.1')&&app.includes('Runtime Data v0.15.0'),'current authority marker missing');
must(index.includes('gl-vs-ai-6.33'),'VS AI v6.33 cache marker missing');
...
must(stack.local_ai==='v6.33'&&stack.tutorial==='v0.59'&&stack.pvp_railway==='v3.40','cross-release metadata stale');
...
console.log('PASS VS AI v6.33 current release: Source Stack v1.8.1 / Playtest Lab v0.14, 200 cards, Shard Deck/Shard Pool, current assets, deck/resource/response/Legacy/Range+Area locks.');
