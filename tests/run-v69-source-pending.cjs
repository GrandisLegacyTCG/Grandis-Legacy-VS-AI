...
assert.ok(vsReducerSource.includes('CONFIRM_RESPONSE_PAYMENT') && vsReducerSource.includes('response_payment'), 'VS AI v6.28 Response commit/payment reducer framework missing');
assert.ok(!tutorialReducerSource.includes('CONFIRM_RESPONSE_PAYMENT'), 'Tutorial v0.54 must remain on its prior response lifecycle in this delivery');
const tutorialData = JSON.parse(fs.readFileSync(path.join(root, 'tutorial/data/season1/cards.runtime.v0.14.2.json'), 'utf8'));
assert.strictEqual(tutorialData.canonical_registry_hash, '5d362f3c1dd785af82f12297d6ab1ecea4f6c43508a7b0f48319e846dd61139c', 'Tutorial v0.54 source baseline changed unexpectedly');
