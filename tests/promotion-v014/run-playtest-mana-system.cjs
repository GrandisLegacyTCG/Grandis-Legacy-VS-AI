...
if(!result.paymentBatchOrder||!result.matchingClassLast)throw Error('Shard payment-batch ordering failed');
if(!result.laterBatchBelowEarlier)throw Error('Later Shard payment batch did not append below earlier batch');
...
if(!app.includes('function returnManaPaymentBatchToOwnerDeck')||!app.includes('generic.concat(nonmatching,matching)'))throw Error('Shard payment-batch return helper missing');
if(app.includes('bottom_locked=true;deck.push(shard)')||app.includes('deck.splice(bottomStart,0,shard)'))throw Error('Obsolete permanent deepest-Class-Shard return logic remains');
