...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.44.json'),'utf8'));
...
assert.strictEqual(lock.one_source_authority,'v1.5.0');
assert.strictEqual(lock.application_runtime_sync,'v2.44');
...
assert.strictEqual(lock.local_ai,'v6.1');
console.log('PASS Application Runtime Sync v2.44 VS AI v6.1 source/deploy lock');
