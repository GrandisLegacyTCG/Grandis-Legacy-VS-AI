...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.27.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.27');
...
assert.strictEqual(lock.local_ai,'v5.42');
console.log('PASS Application Runtime Sync v2.27 VS AI v5.42 source/deploy lock');
