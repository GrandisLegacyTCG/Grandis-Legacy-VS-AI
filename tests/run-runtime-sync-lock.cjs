...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.36.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.36');
...
assert.strictEqual(lock.local_ai,'v5.51');
console.log('PASS Application Runtime Sync v2.36 VS AI v5.51 source/deploy lock');
