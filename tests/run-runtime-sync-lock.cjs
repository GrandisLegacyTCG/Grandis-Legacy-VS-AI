...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.30.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.30');
...
assert.strictEqual(lock.local_ai,'v5.45');
console.log('PASS Application Runtime Sync v2.30 VS AI v5.45 source/deploy lock');
