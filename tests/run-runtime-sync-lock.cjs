...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.28.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.28');
...
assert.strictEqual(lock.local_ai,'v5.43');
console.log('PASS Application Runtime Sync v2.28 VS AI v5.43 source/deploy lock');
