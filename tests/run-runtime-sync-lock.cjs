...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.40.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.40');
...
assert.strictEqual(lock.local_ai,'v5.53');
console.log('PASS Application Runtime Sync v2.40 VS AI v5.53 source/deploy lock');
