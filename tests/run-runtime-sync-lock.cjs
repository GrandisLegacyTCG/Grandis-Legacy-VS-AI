...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.39.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.39');
...
assert.strictEqual(lock.local_ai,'v5.52');
console.log('PASS Application Runtime Sync v2.39 VS AI v5.52 source/deploy lock');
