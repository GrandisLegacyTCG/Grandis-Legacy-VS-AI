...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.29.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.29');
...
assert.strictEqual(lock.local_ai,'v5.44');
console.log('PASS Application Runtime Sync v2.29 VS AI v5.44 source/deploy lock');
