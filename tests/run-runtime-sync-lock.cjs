...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.31.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.31');
...
assert.strictEqual(lock.local_ai,'v5.46');
console.log('PASS Application Runtime Sync v2.31 VS AI v5.46 source/deploy lock');
