...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.32.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.32');
...
assert.strictEqual(lock.local_ai,'v5.47');
console.log('PASS Application Runtime Sync v2.32 VS AI v5.47 source/deploy lock');
