...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.25.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.25');
...
assert.strictEqual(lock.local_ai,'v5.40');
console.log('PASS Application Runtime Sync v2.25 VS AI v5.40 source/deploy lock');
