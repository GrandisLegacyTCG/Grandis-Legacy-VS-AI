...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.26.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.26');
...
assert.strictEqual(lock.local_ai,'v5.41');
console.log('PASS Application Runtime Sync v2.26 VS AI v5.41 source/deploy lock');
