...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.51.json'),'utf8'));
...
assert.strictEqual(lock.one_source_authority,'v1.7.3');
assert.strictEqual(lock.application_runtime_sync,'v2.51');
...
assert.strictEqual(lock.local_ai,'v6.23');
assert.strictEqual(lock.tutorial,'v0.51');
assert.strictEqual(lock.pvp_reference,'v3.20');
...
console.log('PASS Application Runtime Sync v2.51 VS AI v6.23 source/deploy lock');
