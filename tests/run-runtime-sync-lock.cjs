...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.47.json'),'utf8'));
...
assert.strictEqual(lock.one_source_authority,'v1.6.1');
assert.strictEqual(lock.application_runtime_sync,'v2.47');
...
assert.strictEqual(lock.local_ai,'v6.10');
assert.strictEqual(lock.tutorial,'v0.41');
assert.strictEqual(lock.pvp_reference,'v3.08');
...
console.log('PASS Application Runtime Sync v2.47 VS AI v6.10 source/deploy lock');
