...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.49.json'),'utf8'));
...
assert.strictEqual(lock.one_source_authority,'v1.7.1');
assert.strictEqual(lock.application_runtime_sync,'v2.49');
...
assert.strictEqual(lock.local_ai,'v6.15');
assert.strictEqual(lock.tutorial,'v0.43');
assert.strictEqual(lock.pvp_reference,'v3.12');
...
console.log('PASS Application Runtime Sync v2.49 VS AI v6.15 source/deploy lock');
