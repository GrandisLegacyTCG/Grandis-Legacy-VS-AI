...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.33.json'),'utf8'));
...
assert.strictEqual(lock.application_runtime_sync,'v2.33');
...
assert.strictEqual(lock.local_ai,'v5.48');
console.log('PASS Application Runtime Sync v2.33 VS AI v5.48 source/deploy lock');
