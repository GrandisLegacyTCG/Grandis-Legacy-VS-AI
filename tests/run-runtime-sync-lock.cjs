...
const lock=JSON.parse(fs.readFileSync(path.join(root,'sync/runtime-sync-lock.v2.48.json'),'utf8'));
...
assert.strictEqual(lock.one_source_authority,'v1.7.0');
assert.strictEqual(lock.application_runtime_sync,'v2.48');
...
assert.strictEqual(lock.local_ai,'v6.12');
assert.strictEqual(lock.tutorial,'v0.42');
assert.strictEqual(lock.pvp_reference,'v3.10');
...
console.log('PASS Application Runtime Sync v2.48 VS AI v6.12 source/deploy lock');
