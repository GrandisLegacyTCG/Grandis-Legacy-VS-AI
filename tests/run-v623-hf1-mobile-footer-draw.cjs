...
assert(fs.readFileSync(path.join(root,'index.html'),'utf8').includes('gl-vs-ai-6.23-mobile-hf1'));
assert(fs.readFileSync(path.join(root,'tutorial/index.html'),'utf8').includes('gl-tutorial-0.51-mobile-hf1'));
assert.strictEqual(require('../package.json').version,'6.23.0');assert.strictEqual(require('../tutorial/package.json').version,'0.51.0');
console.log('PASS VS AI v6.23 / Tutorial v0.51 Mobile UI HF2');
