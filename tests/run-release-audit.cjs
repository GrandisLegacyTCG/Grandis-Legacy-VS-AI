...
const lock=json('sync/runtime-sync-lock.v2.44.json');assert.strictEqual(lock.canonical_registry_hash,H);assert.strictEqual(lock.application_runtime_sync,'v2.44');assert.strictEqual(lock.local_ai,'v6.0');
...
console.log('PASS VS AI v6.0 release audit: Casting survives Draw/Rank Up, release uses current Hero, response hover diagnostics, mobile resources, and desktop cleanup.');
