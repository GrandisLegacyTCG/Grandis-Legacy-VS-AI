...
const data=json('data/season1/cards.runtime.v0.12.6.json');
const recipes=json('data/season1/effect-recipes.runtime.v0.11.6.json');
const legality=json('data/season1/legality-map.runtime.v0.11.5.json');
...
assert.ok(/runtime_data":"v0\.12\.6/.test(stat)&&/effect_recipe":"v0\.11\.6/.test(stat)&&/runtime_foundation":"v1\.78/.test(stat)&&/runtime_core":"v0\.46/.test(stat),'source stack versions');
...
const lock=json('sync/runtime-sync-lock.v2.36.json');assert.strictEqual(lock.canonical_registry_hash,H);assert.strictEqual(lock.application_runtime_sync,'v2.36');assert.strictEqual(lock.local_ai,'v5.51');
...
console.log('PASS VS AI v5.51 release audit: Casting survives Draw/Rank Up, release uses current Hero, response hover diagnostics, mobile resources, and desktop cleanup.');
