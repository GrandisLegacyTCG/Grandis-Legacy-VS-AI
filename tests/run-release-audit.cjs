...
const lock=json('sync/runtime-sync-lock.v2.43.json');assert.strictEqual(lock.canonical_registry_hash,H);assert.strictEqual(lock.application_runtime_sync,'v2.43');assert.strictEqual(lock.local_ai,'v5.57');
...
assert.ok(fs.existsSync(path.join(root,'tutorial/index.html'))&&app.includes('aiLobbyTutorialButton'),'combined Tutorial application/navigation');

console.log('PASS VS AI v5.57 release audit: Casting survives Draw/Rank Up, release uses current Hero, response hover diagnostics, mobile resources, and desktop cleanup.');
