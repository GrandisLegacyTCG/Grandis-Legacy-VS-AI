...
assert.ok(/runtime_data":"v0\.12\.5/.test(stat)&&/effect_recipe":"v0\.11\.5/.test(stat)&&/runtime_foundation":"v1\.75/.test(stat)&&/runtime_core":"v0\.42/.test(stat),'source stack versions');
...
const lock=json('sync/runtime-sync-lock.v2.29.json');assert.strictEqual(lock.canonical_registry_hash,H);assert.strictEqual(lock.application_runtime_sync,'v2.29');assert.strictEqual(lock.local_ai,'v5.44');
...
console.log('PASS VS AI v5.44 release audit: Casting survives Draw/Rank Up, release uses current Hero, response hover diagnostics, mobile resources, and desktop cleanup.');
