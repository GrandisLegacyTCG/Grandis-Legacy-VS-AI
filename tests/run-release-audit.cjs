...
assert.ok(/runtime_data":"v0\.12\.5/.test(stat)&&/effect_recipe":"v0\.11\.5/.test(stat)&&/runtime_foundation":"v1\.74/.test(stat)&&/runtime_core":"v0\.41/.test(stat),'source stack versions');
...
const lock=json('sync/runtime-sync-lock.v2.25.json');assert.strictEqual(lock.canonical_registry_hash,H);assert.strictEqual(lock.application_runtime_sync,'v2.25');assert.strictEqual(lock.local_ai,'v5.40');
...
assert.ok(/responseDisplayItemsFor/.test(app)&&/data-response-unavailable/.test(app)&&/straight mobile hands/.test(css),'v5.39 response diagnostics and mobile hand lock');
assert.ok(fs.existsSync(path.join(root,'deck-builder/js/data.js'))&&/2.0-public-deck-builder/.test(read('deck-builder/js/data.js')),'bundled Deck Builder v2.0');

console.log('PASS VS AI v5.40 release audit: response diagnostics, Deck Builder v2.0, straight mobile hands, compact mobile Hero indicators, and current runtime/card assets.');
