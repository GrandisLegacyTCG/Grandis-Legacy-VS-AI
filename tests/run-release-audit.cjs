...
const data=json('data/season1/cards.runtime.v0.14.1.json');
const recipes=json('data/season1/effect-recipes.runtime.v0.13.1.json');
...
const lock=json('sync/runtime-sync-lock.v2.50.json');assert.strictEqual(lock.canonical_registry_hash,H);assert.strictEqual(lock.hero_component_registry_hash,HH);assert.strictEqual(lock.application_runtime_sync,'v2.50');assert.strictEqual(lock.local_ai,'v6.21');
...
console.log('PASS VS AI v6.21 release audit: Source Stack 2026-08-24, Hero Components, runtime locks, and preserved UI contracts.');
