...
const runtimeData=JSON.parse(fs.readFileSync(path.join(root,'data/season1/cards.runtime.v0.14.2.json'),'utf8'));const hookCard=(runtimeData.cards||runtimeData).find(x=>x.card_id==='S1-ITM-012');const hookPolicy=rules.reactionPolicyForCard(hookCard);
