...
must(pkg.version==='6.40.0','VS AI package must be v6.40.0');
must(tutPkg.version==='0.66.0','Tutorial package must be v0.66.0');
...
must(app.includes('Grandis Legacy VS AI v6.40'),'VS AI v6.40 marker missing');
must(app.includes('One Source v1.8.2')&&app.includes('Runtime Data v0.15.0'),'current authority marker missing');
must(index.includes('gl-vs-ai-6.40'),'VS AI v6.40 cache marker missing');
...
must(app.includes('responsiveDeviceFamily()')&&app.includes('deviceScreenShortSide()')&&app.includes("gl-ui-tablet")&&app.includes("gl-ui-mobile"),'stable phone/tablet/desktop device-family split missing');
...
must(stack.local_ai==='v6.40'&&stack.tutorial==='v0.66'&&stack.pvp_railway==='v3.42','cross-release metadata stale');
...
must(css.includes('field--opponent .mobile-shard-pool{grid-row:2')||css.includes('field--opponent .mobile-shard-pool{grid-row:2!important'),'Opponent mobile Shard Pool must be above opponent Heroes');
must(css.includes('body:has(.v96-app.mobile-restored-layout) #glMobileAppMenuButton'),'match-time mobile hamburger hide rule missing');
must(css.includes('.attachment-row>.slot .gl-attachment-die'),'mobile Attachment counter containment missing');
console.log('PASS VS AI v6.40 current release: stable device-family split, mobile cleanup, Source Stack v1.8.2 gameplay, Shard payment batches and Triple Shot validation.');
