...
function main(){
 ...
 const counts=rows.reduce((o,r)=>(o[r.coverage_status]=(o[r.coverage_status]||0)+1,o),{}),summary={schema_version:'1.5.0',generated_from:{cards:path.basename(cp),recipes:path.basename(rp)},canonical_registry_hash:cards.raw.canonical_registry_hash||null,card_count:rows.length,recipe_count:recipes.list.length,counts_by_coverage_status:counts,rows};
 const out=a.out||path.join(root,'docs','coverage');fs.mkdirSync(out,{recursive:true});fs.writeFileSync(path.join(out,'SEASON1_CARD_COVERAGE_MATRIX_v1.5.0.json'),JSON.stringify(summary,null,2)+'\n');const headers=Object.keys(rows[0]||{});fs.writeFileSync(path.join(out,'SEASON1_CARD_COVERAGE_MATRIX_v1.5.0.csv'),[headers.join(','),...rows.map(r=>headers.map(h=>csv(r[h])).join(','))].join('\n')+'\n');fs.writeFileSync(path.join(out,'SEASON1_CARD_COVERAGE_SUMMARY_v1.5.0.md'),`# Season 1 Structured Runtime Coverage v1.5.0\n\n- Cards: ${rows.length}\n- Effect recipes: ${recipes.list.length}\n- Executable: ${counts.executable||0}\n- Blocked: ${counts.blocked||0}\n- Canonical registry: \`${summary.canonical_registry_hash||'n/a'}\`\n`);console.log(`PASS structured coverage ${counts.executable||0}/${rows.length}`);if(rows.length!==200||recipes.list.length!==200||(counts.blocked||0)>0)process.exit(1);
}
