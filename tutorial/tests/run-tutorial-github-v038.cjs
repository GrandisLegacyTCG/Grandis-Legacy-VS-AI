...
must(index.includes('gl-tutorial-0.38'),'v0.38 cache revision missing');
...
must(bridge&&bridge.version==='tutorial-bridge-v0.38','bridge version mismatch');
must(qa&&qa.version==='0.31','guide QA version mismatch');
must(app.includes('Grandis Legacy Tutorial v0.38 GitHub Pages')&&app.includes('VS AI v6.2 Base'),'app marker mismatch');
...
must(css.includes('Grandis Legacy Tutorial Guide v0.38'),'tutorial CSS marker mismatch');

const lock=JSON.parse(read('sync/tutorial-github-lock.v0.38.json'));
must(lock.tutorial==='v0.38'&&lock.base_vs_ai==='v6.2'&&lock.delivery==='GitHub Pages','tutorial lock version mismatch');
...
console.log('PASS Grandis Legacy Tutorial v0.38 GitHub Pages: VS AI v6.2 presentation parity with tutorial-only lesson flow preserved.');
