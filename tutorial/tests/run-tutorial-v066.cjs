...
must(pkg.version==='0.66.0','Tutorial package must be v0.66.0');
must(index.includes('gl-tutorial-0.66'),'Tutorial v0.66 cache marker missing');
must(index.includes('Non-Scripted Tutorial Gameplay'),'Tutorial page title missing');
must(app.includes('Grandis Legacy Tutorial v0.66 GitHub Pages'),'Tutorial app marker missing');
must(app.includes('VS AI v6.40 Base'),'VS AI v6.40 shared base marker missing');
must(app.includes('One Source v1.8.2')&&app.includes('Runtime Data v0.15.0'),'current source/runtime markers missing');
must(guide.startsWith('/* Grandis Legacy Tutorial Guide v0.66'), 'Tutorial guide v0.66 marker missing');
must(guideCss.includes('Grandis Legacy Tutorial Guide v0.66'),'Tutorial CSS v0.66 marker missing');
...
must(staticData.includes('"local_ai":"v6.40"')&&staticData.includes('"tutorial":"v0.66"')&&staticData.includes('"pvp_railway":"v3.42"'),'release metadata in static data is stale');
...
const lockPath='sync/tutorial-github-lock.v0.66.json';
must(fs.existsSync(path.join(root,lockPath)),'Tutorial v0.66 release lock missing');
const lock=JSON.parse(read(lockPath));
must(lock.tutorial==='v0.66'&&lock.base_vs_ai==='v6.40'&&lock.source_stack_bundle==='v1.8.2','Tutorial v0.66 lock metadata mismatch');
...
console.log('PASS Grandis Legacy Tutorial v0.66: VS AI v6.40 parity, automatic Draw-to-Deploy after teaching, stable device-family UI split, Shard teaching and release lock verified.');
