...
must(app.includes('Grandis Legacy Tutorial v0.57 GitHub Pages')&&app.includes('VS AI v6.31 Base'),'app marker mismatch');
...
must(css.includes('Grandis Legacy Tutorial Guide v0.57'),'tutorial CSS marker mismatch');
...
const lock=JSON.parse(read('sync/tutorial-github-lock.v0.57.json'));
must(lock.tutorial==='v0.57'&&lock.base_vs_ai==='v6.31'&&lock.delivery==='GitHub Pages','tutorial lock version mismatch');
...
console.log('PASS Grandis Legacy Tutorial v0.57 GitHub Pages: VS AI v6.11 presentation parity with tutorial-only lesson flow preserved.');
