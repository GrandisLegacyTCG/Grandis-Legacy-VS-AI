...
must(index.includes('gl-tutorial-0.46'),'v0.43 cache revision missing');
...
must(app.includes('Grandis Legacy Tutorial v0.46 GitHub Pages')&&app.includes('VS AI v6.18 Base'),'app marker mismatch');
...
must(css.includes('Grandis Legacy Tutorial Guide v0.46'),'tutorial CSS marker mismatch');
...
const lock=JSON.parse(read('sync/tutorial-github-lock.v0.46.json'));
must(lock.tutorial==='v0.46'&&lock.base_vs_ai==='v6.18'&&lock.delivery==='GitHub Pages','tutorial lock version mismatch');
...
console.log('PASS Grandis Legacy Tutorial v0.46 GitHub Pages: VS AI v6.11 presentation parity with tutorial-only lesson flow preserved.');
