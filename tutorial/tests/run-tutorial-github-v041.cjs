...
must(index.includes('gl-tutorial-0.43'),'v0.43 cache revision missing');
...
must(app.includes('Grandis Legacy Tutorial v0.43 GitHub Pages')&&app.includes('VS AI v6.15 Base'),'app marker mismatch');
...
must(css.includes('Grandis Legacy Tutorial Guide v0.43'),'tutorial CSS marker mismatch');
...
const lock=JSON.parse(read('sync/tutorial-github-lock.v0.43.json'));
must(lock.tutorial==='v0.43'&&lock.base_vs_ai==='v6.15'&&lock.delivery==='GitHub Pages','tutorial lock version mismatch');
...
console.log('PASS Grandis Legacy Tutorial v0.43 GitHub Pages: VS AI v6.11 presentation parity with tutorial-only lesson flow preserved.');
