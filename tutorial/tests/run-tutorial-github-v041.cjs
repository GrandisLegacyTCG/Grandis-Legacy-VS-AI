...
must(app.includes('Grandis Legacy Tutorial v0.51 GitHub Pages')&&app.includes('VS AI v6.23 Base'),'app marker mismatch');
...
must(css.includes('Grandis Legacy Tutorial Guide v0.51'),'tutorial CSS marker mismatch');
...
const lock=JSON.parse(read('sync/tutorial-github-lock.v0.51.json'));
must(lock.tutorial==='v0.51'&&lock.base_vs_ai==='v6.23'&&lock.delivery==='GitHub Pages','tutorial lock version mismatch');
...
console.log('PASS Grandis Legacy Tutorial v0.51 GitHub Pages: VS AI v6.11 presentation parity with tutorial-only lesson flow preserved.');
