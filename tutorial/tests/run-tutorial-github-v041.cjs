...
must(app.includes('Grandis Legacy Tutorial v0.55 GitHub Pages')&&app.includes('VS AI v6.24 Base'),'app marker mismatch');
...
must(css.includes('Grandis Legacy Tutorial Guide v0.55'),'tutorial CSS marker mismatch');
...
const lock=JSON.parse(read('sync/tutorial-github-lock.v0.55.json'));
must(lock.tutorial==='v0.55'&&lock.base_vs_ai==='v6.24'&&lock.delivery==='GitHub Pages','tutorial lock version mismatch');
...
console.log('PASS Grandis Legacy Tutorial v0.55 GitHub Pages: VS AI v6.11 presentation parity with tutorial-only lesson flow preserved.');
