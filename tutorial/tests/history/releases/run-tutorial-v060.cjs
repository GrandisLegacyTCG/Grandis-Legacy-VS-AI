...
must(pkg.version==='0.60.0','Tutorial package must be v0.60.0');
must(index.includes('gl-tutorial-0.60'),'Tutorial v0.60 cache marker missing');
must(index.includes('Non-Scripted Tutorial Gameplay'),'Tutorial page title missing');
must(app.includes('Grandis Legacy Tutorial v0.60 GitHub Pages'),'Tutorial app marker missing');
must(app.includes('VS AI v6.34 Base'),'VS AI v6.34 shared base marker missing');
must(app.includes('One Source v1.8.1')&&app.includes('Runtime Data v0.15.0'),'current source/runtime markers missing');
must(guide.startsWith('/* Grandis Legacy Tutorial Guide v0.60'), 'Tutorial guide v0.60 marker missing');
must(guideCss.includes('Grandis Legacy Tutorial Guide v0.60'),'Tutorial CSS v0.60 marker missing');
...
for(const term of [
  ...
  'top of your Shard Deck','Shard Pool maximum of 12','bottom of their owner’s Shard Deck',
...
must(staticData.includes('"local_ai":"v6.34"')&&staticData.includes('"tutorial":"v0.60"')&&staticData.includes('"pvp_railway":"v3.40"'),'release metadata in static data is stale');
...
const lockPath='sync/tutorial-github-lock.v0.60.json';
must(fs.existsSync(path.join(root,lockPath)),'Tutorial v0.60 release lock missing');
const lock=JSON.parse(read(lockPath));
must(lock.tutorial==='v0.60'&&lock.base_vs_ai==='v6.34'&&lock.source_stack_bundle==='v1.8.1','Tutorial v0.60 lock metadata mismatch');
...
console.log('PASS Grandis Legacy Tutorial v0.60: VS AI v6.34 / Playtest Lab v0.14 parity, interactive Shard teaching, current terminology, and release lock verified.');
