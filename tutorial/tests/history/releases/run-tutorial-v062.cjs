...
must(pkg.version==='0.62.0','Tutorial package must be v0.62.0');
must(index.includes('gl-tutorial-0.62'),'Tutorial v0.62 cache marker missing');
must(index.includes('Non-Scripted Tutorial Gameplay'),'Tutorial page title missing');
must(app.includes('Grandis Legacy Tutorial v0.62 GitHub Pages'),'Tutorial app marker missing');
must(app.includes('VS AI v6.36 Base'),'VS AI v6.36 shared base marker missing');
must(app.includes('One Source v1.8.2')&&app.includes('Runtime Data v0.15.0'),'current source/runtime markers missing');
must(guide.startsWith('/* Grandis Legacy Tutorial Guide v0.62'), 'Tutorial guide v0.61 marker missing');
must(guideCss.includes('Grandis Legacy Tutorial Guide v0.62'),'Tutorial CSS v0.61 marker missing');
...
must(staticData.includes('"local_ai":"v6.36"')&&staticData.includes('"tutorial":"v0.62"')&&staticData.includes('"pvp_railway":"v3.41"'),'release metadata in static data is stale');
...
const lockPath='sync/tutorial-github-lock.v0.62.json';
must(fs.existsSync(path.join(root,lockPath)),'Tutorial v0.62 release lock missing');
const lock=JSON.parse(read(lockPath));
must(lock.tutorial==='v0.62'&&lock.base_vs_ai==='v6.36'&&lock.source_stack_bundle==='v1.8.2','Tutorial v0.62 lock metadata mismatch');
...
console.log('PASS Grandis Legacy Tutorial v0.62: VS AI v6.36 / Playtest Lab v0.14 parity, interactive Shard teaching, current terminology, and release lock verified.');
