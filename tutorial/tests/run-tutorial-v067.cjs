...
must(pkg.version==='0.67.0','Tutorial package must be v0.67.0');
must(index.includes('gl-tutorial-0.67'),'Tutorial v0.67 cache marker missing');
must(index.includes('Non-Scripted Tutorial Gameplay'),'Tutorial page title missing');
must(app.includes('Grandis Legacy Tutorial v0.67 GitHub Pages'),'Tutorial app marker missing');
must(app.includes('VS AI v6.41 Base'),'VS AI v6.41 shared base marker missing');
must(app.includes('One Source v1.8.2')&&app.includes('Runtime Data v0.15.0'),'current source/runtime markers missing');
must(guide.startsWith('/* Grandis Legacy Tutorial Guide v0.67'), 'Tutorial guide v0.67 marker missing');
must(guideCss.includes('Grandis Legacy Tutorial Guide v0.67'),'Tutorial CSS v0.67 marker missing');
...
must(app.includes('touch-tablet-legality')&&app.includes('glTabletHandActions'),'tablet Hand legality/action portal missing');
must(read('css/lab-authority.css').includes('flex-direction:column!important'),'tablet action controls must stack vertically');
must(read('css/app.css').includes('translateX(14px)!important'),'mobile Hero horizontal centering correction missing');
must(staticData.includes('"local_ai":"v6.41"')&&staticData.includes('"tutorial":"v0.67"')&&staticData.includes('"pvp_railway":"v3.42"'),'release metadata in static data is stale');
...
const lockPath='sync/tutorial-github-lock.v0.67.json';
must(fs.existsSync(path.join(root,lockPath)),'Tutorial v0.67 release lock missing');
const lock=JSON.parse(read(lockPath));
must(lock.tutorial==='v0.67'&&lock.base_vs_ai==='v6.41'&&lock.source_stack_bundle==='v1.8.2','Tutorial v0.67 lock metadata mismatch');
...
must(app.includes('GL_TUTORIAL_DRAW_LESSON_HOLD'),'Tutorial runtime initial Draw lesson hold missing');
must(app.includes('releaseInitialDrawLesson:function()'),'Tutorial bridge Draw lesson release missing');
must(guide.includes("releaseInitialDrawLesson"),'Draw lesson must release runtime auto-advance');
must(!guide.includes("queuePhaseAdvance('Draw')"),'Draw lesson must not require Next Phase');
console.log('PASS Grandis Legacy Tutorial v0.67: VS AI v6.41 parity, Draw sequencing, Ultimate Tribute teaching, tablet touch UI, Shard teaching and release lock verified.');
